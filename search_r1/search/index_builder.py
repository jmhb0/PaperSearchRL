...
class Index_Builder:
    ...
    def __init__(self,
        ...
        # Limit corpus to 100 elements if test_100 is enabled
        if self.test_100:
            print(
                f"TEST MODE: Limiting corpus to 100 elements (original size: {len(self.corpus)})"
            )
            self.corpus = self.corpus.select(range(min(100, len(self.corpus))))
            print(f"TEST MODE: New corpus size: {len(self.corpus)}")
    ...
    def build_bm25_index(self):
        ...
        pyserini_args = [
            "--collection", "JsonCollection", "--input", temp_dir, "--index",
            self.save_dir, "--generator", "DefaultLuceneDocumentGenerator",
            "--threads", "1"
        ]

        subprocess.run(["python", "-m", "pyserini.index.lucene"] +
                       pyserini_args)
    ...
    def _load_embedding(self, embedding_path, corpus_size, hidden_size):
        all_embeddings = np.memmap(embedding_path, mode="r",
                                   dtype=np.float32).reshape(
                                       corpus_size, hidden_size)
        return all_embeddings

    def _save_embedding(self, all_embeddings):
        memmap = np.memmap(self.embedding_save_path,
    ...
    def encode_all(self):
        ...
        for start_idx in tqdm(range(0, len(self.corpus), self.batch_size),
            ...
            batch_data = self.corpus[start_idx:start_idx +
                                     self.batch_size]['contents']
    ...
    def build_dense_index(self):
        ...
        if self.embedding_path is not None:
            ...
            all_embeddings = self._load_embedding(self.embedding_path,
                                                  corpus_size, hidden_size)
        else:
        ...
        faiss_index = faiss.index_factory(dim, self.faiss_type,
...
def main():
    ...
    # Test parameter
    parser.add_argument('--test_100',
                        action='store_true',
                        default=False,
                        help='Limit corpus to 100 elements for testing')
    ...
    index_builder = Index_Builder(retrieval_method=args.retrieval_method,
